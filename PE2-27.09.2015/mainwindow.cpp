      ...
      ui(new Ui::MainWindow)
{
   ...
   // ui->scrollAreaWidgetContents_2->str
    ui->scrollArea->setMaximumWidth(325);
...
{
    if(QMessageBox::question(this,"Удаление ученика","Вы уверены, что хотите совершить это действие?")==QMessageBox::Yes)
    {
        del(c->number);
        table_repaint();
    }
...
void MainWindow::add_edit_button()
{
    ...
    {
        but->setHidden(1);
        if(glonum ==pupils.size()) write(new Unit(ui->lineEdit->text(), ui->lineEdit_2->text(),pupils.size(),whatyear(),whatsex(),this),pupils.size());
        else write(new Unit(ui->lineEdit->text(), ui->lineEdit_2->text(),pupils.size(),whatyear(),whatsex(),this),glonum);
...
void MainWindow::cellstatbar(int x, int y)
{
